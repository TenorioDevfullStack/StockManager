...
const App = {
  ...
  async init() {
    ...
    if (!DB.remoteReady) {
      this.showAuthMessage(
        DB.remoteError ||
          "Supabase não está configurado. Configure o acesso antes de usar o sistema.",
        "error",
      );
      return;
    }
    ...
    DB.supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "SIGNED_IN" && session?.user)
        await this.startAuthenticated(session.user);
  ...
  bindAuthActions() {
    ...
    document
      ...
      ?.addEventListener("click", async () => this.signOut());
  },
  ...
  async signIn() {
    if (!DB.supabase) {
      this.showAuthMessage(
        DB.remoteError || "Supabase não está configurado.",
        "error",
      );
      return;
    }
  ...
  async signUp() {
    ...
    if (!email || password.length < 6) {
      this.showAuthMessage(
        "Informe e-mail e senha com pelo menos 6 caracteres.",
        "error",
      );
      return;
    }
    ...
    if (!data?.session) {
      this.showAuthMessage(
        "Acesso criado. Confirme o e-mail antes de entrar.",
        "success",
      );
      return;
    }
  ...
  bindGlobalActions() {
    ...
    document
      ...
      ?.addEventListener("change", (event) => this.importBackup(event));
